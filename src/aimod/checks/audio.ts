...
import {
  ...
  audioIsLongerThanMapIssue, Issue
} from "../issue-templates";

export function checkAudioPresence(mapsetFiles: Mapset["files"], general: Beatmap["general"], issues: Issue[]) {
  if (!mapsetFiles[general.audioFilename])
    issues.push(audioNotPresentIssue(general.audioFilename));
}

export function checkMutedOrLow(currentHitObject: BeatmapObject, issues: Issue[]) {
...
export function checkInvalidAudioType(general: Beatmap["general"], issues: Issue[]) {
  if (isInvalidAudioFormat(general.audioFilename)) {
    issues.push(invalidAudioFileIssue(general.audioFilename));
  }
}

export function checkAudioIsLongerThanMap(objects: BeatmapObject[], audio: HTMLAudioElement, issues: Issue[]) {
